#define _CRT_SECURE_NO_WARNINGS            // use to hide all warnings
#define MAXINODE 50							
#define READ 1
#define WRITE 2
...
int fstat_file(int fd)
{
	...
	printf("Filesize : %d\n",temp->FileSize);
	printf("Filesize Actual size: %d\n",temp->FileActualSize);
	printf("Link count : %d\n",temp->LinkCount);
	printf("Reeference count : %d\n",temp->ReferenceCount);
...
int stat_file(char *name)
{
	...
	printf("\n------Statical Informtfation about file------\n");
	printf("File name : %s\n",temp->FileName);
	printf("Inode Number %d\n",temp->InodeNumber);
	printf("Filesize : %d\n",temp->FileSize);
	printf("Filesize Actual size: %d\n",temp->FileActualSize);
	...
	while(1)
	{
		...
		if(count ==1)
		{
			...
			else if(_stricmp(command[0],"closeall")==0)
			{
				...
				printf("All File successfully closed\n");
				continue;
			}
			...
			else if(_stricmp(command[0],"exit")==0)
			{
					...
					printf("\nYou Do Very Well Task In Virtual File System");
		...
		else if(count ==2)
		{
			...
			if(_stricmp(command[0],"stat")==0)
			{
				...
				if(ret == -2)
					printf("ERROR : Threre is no such file\n");
				continue;
			}
			...
			else if(_stricmp(command[0],"write")==0)
			{
				...
				if(ret == -2)
					printf("ERROR : Threre is no suffiecient memory to write\n");
				if(ret == -3)
		...
		else if(count ==3)
		{
			...
			if(_stricmp(command[0],"create")==0)
			{
				...
				if(ret == -1)
					printf("ERROR : Incorrect parameters\n");
				if(ret == -2)
					printf("ERROR : Threre is no inodes\n");
				if(ret == -3)
